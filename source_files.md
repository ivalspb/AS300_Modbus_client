...
// УДАЛЕНЫ старые обработчики onM11StatusChanged, onM12StatusChanged, onM0M14StatusChanged
// Теперь всё обрабатывается в onStatusRegisterRead()
...
private:
    ...
    void setupStatusMonitoring(); // НОВЫЙ метод
    void checkCompletionCondition(); // НОВЫЙ метод
...
void DataRepository::saveToDatabaseAsync() {
    if (!m_dbManager || m_data.isEmpty()) {
        qDebug() << "DataRepository: Nothing to save";
        return;
    }
